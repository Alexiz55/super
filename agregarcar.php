...
$qry=$conexion->query("SELECT * FROM inventario WHERE id_producto='".$id."'");
$row = $qry->fetch_array();
...
//herramienta array.
$carro[md5($id)]=array('id_producto'=>md5($id),'existencias'=>$existencias,'id'=>$id,
'imagen'=>$row['imagen'],'nombre'=>$row['nombre'],'precio'=>$row['precio']);
